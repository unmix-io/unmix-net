...
def generate(alpha1, alpha2, rate, channels=2):
    ...
    model = Conv2D(32, (12, 3), activation='relu', padding='same')(input)
    # model = ReLU(alpha=alpha1)(model)
    model = Conv2D(16, (12, 3), activation='relu', padding='same')(model)
    # model = LeakyReLU(alpha=alpha1)(model)
    model = MaxPooling2D(pool_size=(12, 1))(model)
    model = Conv2D(64, (12, 3), activation='relu', padding='same')(model)
    # model = LeakyReLU(alpha=alpha1)(model)
    model = Conv2D(32, (12, 3), activation='relu', padding='same')(model)
    # model = LeakyReLU(alpha=alpha1)(model)
    model = MaxPooling2D(pool_size=(12, 1))(model)
    model = Dropout(0.5)(model)
    model = Flatten()(model)
    model = Dense(768, activation='relu', )(model)
    # model = LeakyReLU(alpha=alpha1)(model)
    model = Dropout(0.5)(model)
    model = Dense(384, activation='relu', )(model)
    model = Dense(769)(model)
